...
# ============================================================
#
# sudo bash ./tosca-uploadV2.sh mytosca.yaml 18003 swarmkb default lb >> tst.csv
#
...
query_db_by_template() {
  local base="$1"
  hr; echo "3) Querying KB on ${base} for templateId: ${TEMPLATE_ID}"
  [[ -n "$TEMPLATE_ID" ]] || { echo "No template_id; skipping KB query"; return 0; }
  ...
  echo "KB response:"; echo "$resp" | jq . 2>/dev/null || echo "$resp"
}

...
func (f *FieldDef) Validate(val interface{}) error {
	...
	case TypeText:
		...
		///
	case TypeUUID:
		if reflect.TypeOf(val).Kind() != reflect.String || len(val.(string)) != 36 {
			return ErrTypeMismatch
		}
		///
	case TypeTimestamp:
...
func (r *Tuple) Decode(buf *bytes.Buffer) error {
	for _, fd := range r.Relation.Fields {
		...
		case TypeBigInt:
			...
			v = val
		case TypeVarchar, TypeText, TypeUUID:
			var strLen uint32
			if err := binary.Read(buf, binary.LittleEndian, &strLen); err != nil {
				return err
			}
			...
			//v = string(strBuf) fmt.Sprintf("%s", strBuf)
			v = fmt.Sprintf("%s", strBuf)
		case TypeBoolean:
			var val bool
			if err := binary.Read(buf, binary.LittleEndian, &val); err != nil {
				return err
		...
		case TypeTimestamp:
			var timeLen uint32
			if err := binary.Read(buf, binary.LittleEndian, &timeLen); err != nil {
				return err
			}
			timeBuf := make([]byte, timeLen)
			_, err := buf.Read(timeBuf)
			if err != nil {
				return err
			}
			var timestamp time.Time
			if err := timestamp.UnmarshalBinary(timeBuf); err != nil {
				return err
			}
			v = timestamp
		default:

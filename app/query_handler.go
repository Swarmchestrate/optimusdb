...
func handleQueryStream(stream network.Stream, db *KnowledgeBaseDB) {
	...
	//self := string(db.Node.Identity) //Pretty()
...
func queryPeersLimited(
	...
	for _, pid := range peers {
		go func(p peer.ID) {
			// skip loops
			if contains(tracePath, string(p)) { //Pretty()
			...
			req := map[string]interface{}{
				"criteria":   criteria,
				"trace_id":   traceID,
				"trace_path": append(tracePath, string(db.Node.Identity)), //Pretty()
			...
			for _, r := range rows {
				if _, ok := r["_source"]; !ok {
					r["_source"] = map[string]interface{}{
						"type":    "peer",
						"peer_id": string(p),                                   //Pretty()
						"path":    append(tracePath, string(db.Node.Identity)), //Pretty()
					}
				}
				if _, ok := r["_trace"]; !ok {
					r["_trace"] = map[string]interface{}{
						"id":   traceID,
						"path": append(tracePath, string(db.Node.Identity)),
					}
				}
			}
...
func queryPeersUntilQuorum(ctx context.Context, db *KnowledgeBaseDB, criteria []map[string]interface{}, traceID string, tracePath []string, opts qOptions) ([]map[string]interface{}, error) {
	...
	for _, pid := range allPeers {
		go func(p peer.ID) {
			if contains(tracePath, string(p)) {
				ch <- peerResp{pid: string(p), rows: nil, err: nil}
				return
			}
			stream, err := db.Node.PeerHost.NewStream(ctx, p, "/query/1.0.0")
			if err != nil {
			...
			req := map[string]interface{}{
				"criteria":   criteria,
				"trace_id":   traceID,
				"trace_path": append(tracePath, string(db.Node.Identity)),
				"options": map[string]interface{}{
					"strategy":        "LOCAL_ONLY",
					"include_local":   true,
					"annotate_source": true,
			...
			tagRows(rows, map[string]interface{}{
				...
				"peer_id": string(p),
				"path":    append(tracePath, string(db.Node.Identity)),
			}, traceID, append(tracePath, string(db.Node.Identity)))
			ch <- peerResp{pid: string(p), rows: rows, err: nil}
		}(pid)
	}

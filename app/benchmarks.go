...
// UpdateBootstrap bootstrapping-time is the time it takes a new node to gather all existing data.
...
*/
// The error arises because the time.Time type in Go does not have a Compare method.
//Instead, Go's standard library provides comparison functions through operators and methods
// like Before, After, or equality checks (==, !=).
// To fix this issue, you can rewrite the UpdateBootstrap function using the appropriate Before and After methods provided by time.Time.

func (b *Benchmark) UpdateBootstrap(ts time.Time) {
	if ts.Before(startup_ts) || ts.Equal(startup_ts) {
		fmt.Print("UpdateBootstrap")
		// replicated entry means we have a better approximation
		now := time.Now()
		b.Bootstrap = now.Sub(startup_ts)
...
// UpdateNewContributions To compare two time.Time instances, you should use the Before, After, or Equal methods provided by the time package.
/*

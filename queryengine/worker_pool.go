...
func (wpe *WorkerPoolEngine) queryPeer(
	...
	log.Printf("[WORKER-POOL] Sending query trace=%s to peer %s", traceID[:8], string(peerID)[:8])
	...
	log.Printf("[WORKER-POOL] Received %d results from peer %s", len(results), string(peerID)[:8])
